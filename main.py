...
def extract():
    try:
        ...
        # Demo mode (test frames)
        if demo:
            demo_video = os.path.join(temp_dir, "demo.mp4")
            cmd = ["/usr/bin/ffmpeg", "-f", "lavfi", "-i", "testsrc2=duration=5:size=320x240:rate=4", "-pix_fmt", "yuv420p", "-y", demo_video]
        ...
        # Frame Extraction
        ffmpeg_cmd = ["/usr/bin/ffmpeg", "-i", input_video, "-vf", "fps=4", "-y", frames_pattern]
